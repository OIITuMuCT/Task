...
#     response = client.patch(f"/api/v1/tasks/{task.id}/claim", headers=jwt_token)
...
def test_task_not_found(client, user):
    response = client.patch('/9999999/claim/')
...
def test_unauthorized_claim_attempt(client, user, jwt_token):
    ...
    permission = Permission.objects.get(codename='change_task', content_type=content_type)
